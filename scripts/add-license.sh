...
# EXCLUDE_LIST is an array of files paths to exclude from prepending with a
# license notice.
...
// Copyright $YEAR The MongoPB Authors.
//
...
for file in $(find . -name "*.go" -type f); do
    	...
    	if grep -q "Copyright $YEAR The MongoPB Authors." "${file}"; then
		continue
    	fi
